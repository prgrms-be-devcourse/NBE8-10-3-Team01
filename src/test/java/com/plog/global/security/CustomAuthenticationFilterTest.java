...
class CustomAuthenticationFilterTest {
    ...
    @Mock private AuthService authService; // findMemberWithId를 위해 주입
    ...
    void handleAccessTokenReissue_success() throws Exception {
        ...
        given(rtClaims.getSubject()).willReturn("1");
        given(jwtUtils.parseToken(validRt)).willReturn(rtClaims);

        // 서비스 및 유틸리티 모킹
        MemberInfoRes memberInfo = new MemberInfoRes(1L, "test@plog.com", "plogger", LocalDateTime.now());
        given(authService.findMemberWithId(1L)).willReturn(memberInfo);
        given(jwtUtils.createAccessToken(memberInfo)).willReturn(newAt);

        Claims newAtClaims = mock(Claims.class);
        given(newAtClaims.getSubject()).willReturn("1");
        given(newAtClaims.get("email", String.class)).willReturn("test@plog.com");
